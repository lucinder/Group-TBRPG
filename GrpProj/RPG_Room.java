...
public class RPG_Room {
   ...
   public static void printStaggered(String input, int delayMS) throws InterruptedException { // ONLY works for
   ...
   // Room #1, the starting room, must have a path to every other room in the
   // dungeon with at least 1 door.
   private String roomID = "DefaultRoomID";
   /**
    * Lucinder's note: my aim is to eventually be able to implement a "seed"
    * feature using this class
    ...
    * The nomenclature system will be important if we choose to implement saving
    * and loading as a feature in the future!
    **/
   private String roomType = "Empty"; // Room type: must be one of the following: Empty, Enemy, Trap, Treasure, EnTrap
                                      // (Enemy/Trap), EnTres (Enemy/Treasure), EnTrapTres (Enemy/Trap/Treasure),
                                      // TrapTres (Trap/Treasure), Shop, Boss
   private String dialogue = "";
   private boolean[] exits = { false, false, false, false };
   /**
    * Added 11/18- which sides of the room are functional exits (N, E, S, W)?
    * default none.
    * The empty no-exit default room provides a room that serves as a "wall"- that
    * is, it can be placed between rooms or dungeon layers to prevent direct
    * passage.
    * Using the exit system, we can build our dungeon as a 2d array rather than a
    * LinkedList, but also place certain limits on access even within the linked
    * list structure
    * The reason in general I've made this change is for modularity; randomization
    * of dungeons can be done more easily using conditionals.
    * - All rooms on the main path must have at least 1 exit.
    * - Dead ends- rooms with only 1 exit- must be either boss rooms, shops, or
    * sidequest rooms.
    * - Rooms with 2 exits are connecting paths between central rooms. They can
    * only have traps.
    * - Rooms with 3 or more exits are central rooms. They can contain any
    * combination of traps, encounters, and treasures.
    * - There must be at least 3 dead ends in a dungeon path; the boss room, shop,
    * and sidequest room.
    **/
   private RPG_Room up;
   ...
   private ArrayList<RPG_Interactable> objects = new ArrayList<RPG_Interactable>(); // contains all characters, traps,
                                                                                    // treasure/items, and enemies or
                                                                                    // shopkeeps

   public RPG_Room() throws Exception {
   }

   public RPG_Room(int roomNumber) throws Exception {
      this.roomNumber = "" + roomNumber;
   }

   public RPG_Room(String id) throws InterruptedException, Exception {
      roomID = id;
      String[] idParser = id.split("_");
      roomNumber = idParser[0];
      roomType = idParser[1];
      // once we add a method for processing object id strings- parseObjects(String
      // id)- add that here
      if (roomType.equals("En") || roomType.equals("EnTrap") || roomType.equals("EnTres")
            || roomType.equals("EnTrapTres")) { // add enemies to enemy rooms
         addRandomEnemy();
      }
      if (roomType.equals("Trap") || roomType.equals("EnTrap") || roomType.equals("TrapTres")
            || roomType.equals("EnTrapTres")) { // add traps to trapped rooms
         addRandomTrap();
      }
      if (roomType.equals("Tres") || roomType.equals("EnTres") || roomType.equals("TrapTres")
            || roomType.equals("EnTrapTres")) {
         addTreasure();
      }
   ...
   public RPG_Room(String id, String difficultyOverride) throws InterruptedException, Exception {
      roomID = id;
      String[] idParser = id.split("_");
      roomNumber = idParser[0];
      roomType = idParser[1];
   ...
   public RPG_Room getDown() {
      return down;
   }

   public RPG_Room getUp() {
      return up;
   }

   public RPG_Room getLeft() {
      return left;
   }

   public RPG_Room getRight() {
      return right;
   }
   ...
   public void addRandomEnemy() throws Exception {
      /**
       * int cr = 0;
       * if(roomNumber.charAt(roomNumber.length()-1) == 'n' ||
       * roomNumber.charAt(roomNumber.length()-1) == 's'){
       * cr = Integer.parseInt(roomNumber.substring(0,roomNumber.length()-1)); // cut
       * off last character
       * } else {
      ...
      RPG_Enemy enemy = new RPG_Enemy(RPG_Enemies_List.floorOne[enemyID]); // send an existing enemy to a copy
                                                                           // constructor so each can be treated
                                                                           // differently
      objects.add(enemy); // add enemy to objects array
   }

   public void addRandomEnemy(String difficultyOverride) throws InterruptedException {
      RPG_Enemy enemy;
   ...
   public void addRandomBoss() throws InterruptedException {
      objects.add(new RPG_Enemy(RPG_Enemies_List.Necromancer));
   }

   public void addRandomBoss(String difficultyOverride) throws InterruptedException {
      RPG_Enemy boss;
      if (difficultyOverride.equals("Easy")) { // easy mode- all enemies are normal dummies
