...
public class RPG_Dungeon{
   ...
   public RPG_Dungeon(RPG_Room head, RPG_Player p) throws IOException,Exception{
      player = p;
   ...
   public static void intro2(int levels, int levelLength){
      try{
         ...
         printStaggered("When you open your eyes again, you are in the very bottom of the dungeon, in a bleak and cavernous room adorned only by moss growing on the walls.");
         return;
      } catch (Exception e){
         e.printStackTrace();
      }
   ...
   public void mainLoop() throws IOException,Exception{
      ...
      if(contains(action,"inventory")){ // check inventory
      ...
      } else if (contains(action,"shop") || contains(action,"buy") || contains(action, "sell")){
         boolean shopSuccessful = false;
         for(RPG_Interactable i : DUNGEON_ROOM_CURRENT.getObjects()){
            if(i instanceof RPG_Shop){
               RPG_Shop shop = (RPG_Shop)i;
               shop.shop(player); // shop da shop
               shopSuccessful = true;
            }
         }
         if(!shopSuccessful){
            printStaggered("You tried to do some shopping... but there were no stores in sight!");
         }
      ...
      } else if(contains(action, "mp") || contains(action, "mana")) {
         printStaggered("Your current MP is [" + player.getMP() + "/" + player.getMaxMP() + "].");
      } else if(player.getPlayerClass() instanceof RPG_Spellcaster && (contains(action, "spell") || contains(action, "spells"))){
         RPG_Spellcaster caster = (RPG_Spellcaster)player.getPlayerClass();
         caster.printSpellList();
         ...
         player.die(); // funney :)
      } else if(contains(action, "moss")){
         printStaggered("You ate some of the moss off the walls.");
         printStaggered("It tasted great!");
      } else {
   ...
   public void loadRoomContents() throws IOException,InterruptedException,Exception{
      if(DUNGEON_ROOM_CURRENT.isFirstVisit()){ DUNGEON_ROOM_CURRENT.printSpecialDialogue(); DUNGEON_ROOM_CURRENT.visit();}
      for(RPG_Interactable i : DUNGEON_ROOM_CURRENT.getObjects()){
         i.interactionEvent();
         ...
         if(i instanceof RPG_Enemy){ // enemy detected?
            ...
            if(!(en.isPacified()) && en.getHP() > 0){
               if(i.getName().equals("Necromancer")){ boss1Intro(); }
   ...
   **/
   ...
   public void boss1Intro() throws InterruptedException{
      printStaggered("\"Welcome to my laboratory, Hero. I am Lucius, a necromancer serving the Monster King.\"");
      printStaggered(" [0] \"What are you doing to that corpse?\"\n [1] Attack the Necromancer");
      ...
      while(!selectionmade){
         try{
            ...
            if(action > 1 || action < 0){ throw new Exception("Out of bounds!"); }
         } catch (Exception e){
            printStaggered("Invalid selection!");
            selectionmade = false;
         }
      ...
      if(action == 0){
         printStaggered("\"I\'m trying to resurrect my wife back from the dead.\"");
         printStaggered("\"You see, I used to be one of the high priests under the King, serving Veritas diligently.\"");
         printStaggered("\"However, the King lusted for my wife, and secretly kidnapped her when I was away for a mission.\"");
         printStaggered("\"Furiously, I attacked the King, but was only able to recover the corpse of my wife with my life hanging by a thread.\"");
         printStaggered("\"That was when the Monster King led me to the dungeon.\"");
         printStaggered("\"He gave me the tools necessary to revive her...\"");
         printStaggered("\"...and since your mission is to kill him, I\'ll need to kill you first!\"");
      }
   }
