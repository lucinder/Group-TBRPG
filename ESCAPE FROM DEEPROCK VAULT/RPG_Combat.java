...
public class RPG_Combat{
    ...
    */
    ...
    // Enemy attack method
    public void attack(RPG_Enemy actor) throws Exception{
   ...
   public void attack(RPG_Player actor, RPG_Attack action, boolean doesConsumeItem) throws Exception{
      attack(actor, action);
      // System.out.println("DEBUG - action name = " + action.getName() + ", found in player inventory = " + actor.hasItem(action.getName()));
      actor.removeItem(actor.getItem(action.getName()));
   }
   public void attack(RPG_Player actor, RPG_Character target, RPG_Attack action, boolean doesConsumeItem) throws Exception{
      attack(actor,target,action);
      // System.out.println("DEBUG - action name = " + action.getName() + ", found in player inventory = " + actor.hasItem(action.getName()));
      actor.removeItem(actor.getItem(action.getName()));
   }
   ...
   public void attack(RPG_Player actor) throws Exception,InterruptedException{
      ...
      if(selection == 0){ // attack
         ...
         if(itemSelected){ // early breaks from going back skip this action
            if(itemAction.getType().equals("Healing") || itemAction.getType().equals("Buff")){ // healing heals self
               attack(actor, actor, itemAction, true);
            } else {
               attack(actor, itemAction, true); // pass our selection on directly to action usage; target is automatically self
            }
         }
